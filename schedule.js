const WEEK=[{v:1,s:'周一'},{v:2,s:'周二'},{v:3,s:'周三'},{v:4,s:'周四'},{v:5,s:'周五'},{v:6,s:'周六'},{v:7,s:'周日'}];
...
function timeline(date){const dayRows=rows(date),ss=slots();let html='<div class="timeline timeline-grid">';ss.forEach((s,i)=>{const occupied=slotRows(date,s).length>0;html+=`<div class="slot-time grid-time" style="grid-row:${i+1};grid-column:1">${s}</div><div class="slot-bg ${occupied?'occupied':''}" style="grid-row:${i+1};grid-column:2"></div>`;if(!occupied)html+=`<button class="slot-add" data-time="${s}" style="grid-row:${i+1};grid-column:2">＋ 点击约课</button>`;});dayRows.forEach(x=>{const p=eventPlacement(x);html+=card(x,p.rowStart,p.span)});html+='</div><div class="timeline-end"><span>21:00</span></div>';return html}
function card(x,rowStart=1,span=1){const leave=x.attendanceStatus==='leave',label=leave?'已请假':(isTrial(x)?'首次体验':(kind(x)==='temporary'?'临时约课':'固定课'));return `<div class="course merged-course ${eventClass(x)}" style="grid-row:${rowStart} / span ${span};grid-column:2" data-id="${x.id}"><div class="course-main"><strong>${x.startTime}–${x.endTime} · ${esc(x.studentName)}</strong><small>${esc(x.type||'综合')}${x.notes?` · ${esc(x.notes)}`:''}</small></div><div class="course-side"><span class="tag">${label}</span><div class="course-actions"><button type="button" class="course-leave tiny" data-id="${x.id}" data-date="${x.occurrenceDate}">${leave?'恢复':'请假'}</button><button type="button" class="course-cancel tiny cancel" data-id="${x.id}" data-date="${x.occurrenceDate}">取消</button></div></div></div>`}
function memberList(arr){return arr.length?arr.map(m=>`<button class="member-edit" data-id="${m.id}"><strong>${esc(m.name)}</strong><small>${esc(m.goal||'未填写目标')}${m.contact?` · ${esc(m.contact)}`:''}</small></button>`).join(''):'<div class="empty small">还没有会员，点击右上角“＋会员”创建。</div>'}
function fillMembers(q='',selectedName=''){const a=members.filter(m=>!q||m.name.toLowerCase().includes(q.toLowerCase()));$('studentName').innerHTML=a.map(m=>`<option value="${esc(m.name)}" ${m.name===selectedName?'selected':''}>${esc(m.name)}</option>`).join('')||'<option value="">暂无会员</option>'}
function toggleFields(){const trial=$('appointmentType').value==='trial';if(trial)$('kind').value='temporary';$('scheduleKindField').style.display=trial?'none':'';$('regularStudentFields').style.display=trial?'none':'';$('trialStudentField').style.display=trial?'':'none';$('regularCourseTypeField').style.display=trial?'none':'';const temp=trial||$('kind').value==='temporary';$('fixedField').style.display=temp?'none':'';$('tempField').style.display=temp?'':'none'}
function openCourse(x=null,preset=''){$('courseTitle').textContent=x?'编辑课程':'安排课程';$('courseId').value=x?.id||'';const trial=isTrial(x);$('appointmentType').value=trial?'trial':'regular';$('kind').value=trial?'temporary':kind(x||{});$('studentSearch').value='';fillMembers('',trial?(members[0]?.name||''):(x?.studentName||members[0]?.name||''));$('trialName').value=trial?x.studentName:'';$('weekday').value=x?.day||wd(selected);$('courseDate').value=x?.date||selected;$('startTime').value=x?.startTime||preset||'16:00';$('endTime').value=x?.endTime||(preset?mt(Math.min(tm(preset)+60,1260)):'17:00');$('courseType').value=trial?'综合':(x?.type||'综合');$('notes').value=x?.notes||'';$('deleteBtn').classList.toggle('hidden',!x);toggleFields();$('courseDialog').showModal()}
