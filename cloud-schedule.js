(() => {
  ...
  async function listScheduleContents(){const rows=await rest('trainlog_schedule_contents?select=id,schedule_id,occurrence_date,tests,training,training_notes,created_at,updated_at&order=occurrence_date.asc');return (rows||[]).map(mapContent);}
  async function upsertScheduleContent(x){const rows=await rest('trainlog_schedule_contents?on_conflict=schedule_id,occurrence_date&select=*',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({schedule_id:x.scheduleId,occurrence_date:x.date,tests:x.tests||[],training:x.training||[],training_notes:x.trainingNotes||'',updated_at:new Date().toISOString()})});return mapContent(rows[0]);}
